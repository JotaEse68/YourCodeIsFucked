...
export function applyRefactorOperation(target: string, operation: RefactorOperation, options: { allowSupervised?: boolean } = {}): AppliedOperation { const root = resolve(target); const id = operation.id;
  ...
  if (operation.kind === 'CREATE') { const file = resolve(root, operation.file); if (existsSync(file)) throw new Error(`Destination already exists: ${operation.file}`); { const safety = assessRefactorSafety([file], new Map([[file, operation.content]])); if (safety.mode === 'BLOCKED') throw new Error(`BLOCK: ${safety.reason}`); if (safety.mode === 'SUPERVISED' && !options.allowSupervised) throw new Error(`SUPERVISED: ${safety.reason}`); } mkdirSync(dirname(file), { recursive: true }); atomicWrite(file, operation.content); return { operationId: id, changedFiles: [operation.file], description: operation.description, relatedTestFiles: [], undo: () => { if (existsSync(file)) unlinkSync(file); } }; }
  if (operation.kind === 'DELETE') { const file = resolve(root, operation.file); const content = readFileSync(file, 'utf8'); { const safety = assessRefactorSafety([file], new Map([[file, content]])); if (safety.mode === 'BLOCKED') throw new Error(`BLOCK: ${safety.reason}`); if (safety.mode === 'SUPERVISED' && !options.allowSupervised) throw new Error(`SUPERVISED: ${safety.reason}`); } unlinkSync(file); return { operationId: id, changedFiles: [operation.file], description: operation.description, relatedTestFiles: [], undo: () => { mkdirSync(dirname(file), { recursive: true }); atomicWrite(file, content); } }; }
  if (operation.kind === 'EDIT_IMPORT' || operation.kind === 'EDIT_EXPORT') { const file = resolve(root, operation.file); const before = readFileSync(file, 'utf8'); { const safety = assessRefactorSafety([file], new Map([[file, before]])); if (safety.mode === 'BLOCKED') throw new Error(`BLOCK: ${safety.reason}`); if (safety.mode === 'SUPERVISED' && !options.allowSupervised) throw new Error(`SUPERVISED: ${safety.reason}`); } let after = before; for (const replacement of operation.replacements) after = after.split(replacement.from).join(replacement.to); if (after !== before) atomicWrite(file, after); return { operationId: id, changedFiles: after === before ? [] : [operation.file], description: operation.description, relatedTestFiles: [], undo: () => atomicWrite(file, before) }; }
  if (operation.kind === 'EXTRACT') {
    ...
    { const safety = assessRefactorSafety([source], new Map([[source, before]])); if (safety.mode === 'BLOCKED') throw new Error(`BLOCK: ${safety.reason}`); if (safety.mode === 'SUPERVISED' && !options.allowSupervised) throw new Error(`SUPERVISED: ${safety.reason}`); }
