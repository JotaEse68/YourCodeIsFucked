...
export function moduleImportGraph(target: string): ModuleImportEdge[] {
  const root = resolve(target); const configured = aliases(root);
  return walk(root).map((file) => {
    const imports = new Set<string>();
    for (const match of readFileSync(file, 'utf8').matchAll(importSpecifierPattern)) { const resolved = resolveImport(file, match[2], root, configured); if (resolved) imports.add(relative(root, resolved).replaceAll('\\', '/')); }
    return { file: relative(root, file).replaceAll('\\', '/'), imports: [...imports].sort() };
