...
export function wordpressFindings(displayPath: string, content: string): Finding[] {
  ...
  if (/(?:^|[\\/])wp-config\.php$/i.test(displayPath) || /^wp-config\.php$/i.test(displayPath)) {
    const debugLines = linesMatching(content, /\bdefine\s*\(\s*['"](?:WP_DEBUG|WP_DEBUG_DISPLAY|SCRIPT_DEBUG)['"]\s*,\s*true\s*\)/i);
    if (debugLines.length > 0) findings.push({ id: `wordpress-production-debug-config:${displayPath}`, ruleId: 'wordpress-production-debug-config', severity: debugLines.some((line) => /WP_DEBUG_DISPLAY/.test(content.split(/\r?\n/)[line - 1])) ? 'medium' : 'low', risk: 'architectural', file: displayPath, lines: debugLines, evidence: 'WordPress development debugging is enabled in wp-config.php. Disable visible debug output before production; keep logs outside public web paths if they are needed.', scoreImpact: debugLines.some((line) => /WP_DEBUG_DISPLAY/.test(content.split(/\r?\n/)[line - 1])) ? 5 : 0 });
    const hardcodedPasswordLines = linesMatching(content, /\bdefine\s*\(\s*['"](?:DB_PASSWORD|DB_USER)['"]\s*,\s*['"](?!['"]\s*\))/i);
    if (hardcodedPasswordLines.length > 0) findings.push({ id: `wordpress-hardcoded-config-secret:${displayPath}`, ruleId: 'wordpress-hardcoded-config-secret', severity: 'medium', risk: 'architectural', file: displayPath, lines: hardcodedPasswordLines, evidence: 'A database credential appears to be written directly in wp-config.php. Do not commit or print it; rotate it if the repository was shared and load it from deployment secrets instead.', scoreImpact: 5 });
    const fileEditorLines = linesMatching(content, /\bdefine\s*\(\s*['"]DISALLOW_FILE_EDIT['"]\s*,\s*false\s*\)/i);
    if (fileEditorLines.length > 0) findings.push({ id: `wordpress-file-editor-config:${displayPath}`, ruleId: 'wordpress-file-editor-config', severity: 'medium', risk: 'architectural', file: displayPath, lines: fileEditorLines, evidence: 'The WordPress file editor is explicitly enabled. Disable it in production to reduce the impact of an administrator account compromise.', scoreImpact: 3 });
  }
